...
For demonstration purposes we will work with a synthetic dataset in the following. Spiking activity of 10 hypothetical cells are assumed to be given as iid. Poisson processes. Upon binning with a given bin width, this yields Bernoulli processes in discrete time. We create such Bernoulli data and then insert hypothetical correlations by means of a number of co-activations of different cell groups (also known as cell assemblies).

The following script is contained in the file ``my\_first\_script.py".  We first import the necessary modules into our Python session (we recommend using ipython in pylab mode, i.e. running \texttt{ipython --pylab} to paste and run text copied to the clipboard from this tutorial using the magic command \texttt{\%paste}).
...
# versus the raw data
plt.figure()
plt.scatter(range(len(spikes_model.emperical.sequence)),
		1 + np.array(spikes_model.emperical.sequence))
...
for c, memory in enumerate(bin_memories):
	arr[:, c + len(bin_memories)] = spikes_model.memories.stas[memory] /
